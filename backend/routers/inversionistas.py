    ...
    try:
        ...
        for e in events:
            ...
            cert_data = {
                ...
                'compra_nuevas_cuotas': float(e.get('monto_aumento') or payload.get('aumentos_capital') or 0.0) + float(e.get('monto_capitalizacion', 0.0)),
                'rescates': float(e.get('monto_rescate', 0.0)),
                'monto_transferido': max(0.0, float(e.get('monto_reparto', 0.0) or 0.0) + float(e.get('monto_rescate', 0.0) or 0.0) - float(e.get('penalidad_rescate') or payload.get('penalidades') or 0.0) - float(e.get('monto_deduccion', 0.0) or 0.0)),
