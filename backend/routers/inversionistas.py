    ...
    try:
        ...
        for e in events:
            ...
            cert_data = {
                ...
                'penalidades': float(e.get('penalidad_rescate') or payload.get('penalidades') or 0.0),
                'neto_disponible': float(e.get('interes_neto_disponible', 0.0)),
                'aumentos_capital': float(e.get('monto_aumento') or payload.get('aumentos_capital') or 0.0),
                'capitalizacion': float(e.get('monto_capitalizacion', 0.0)),
                'compra_nuevas_cuotas': float(payload.get('compra_nuevas_cuotas')) if payload.get('compra_nuevas_cuotas') is not None else (float(e.get('monto_aumento') or payload.get('aumentos_capital') or 0.0) + float(e.get('monto_capitalizacion', 0.0))),
                'rescates': float(e.get('monto_rescate', 0.0)),
                'monto_transferido': float(payload.get('monto_transferido_calculado')) if payload.get('monto_transferido_calculado') is not None else max(0.0, float(e.get('monto_reparto', 0.0) or 0.0) + float(e.get('monto_rescate', 0.0) or 0.0) - float(e.get('penalidad_rescate') or payload.get('penalidades') or 0.0) - float(e.get('monto_deduccion', 0.0) or 0.0)),
