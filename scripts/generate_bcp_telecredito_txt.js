...
async function testGenerateBcpTxt() {
  console.log('========================================================');
  console.log('GENERANDO ARCHIVO TXT OFICIAL TELECRÉDITO BCP (ENE-FEB 2026)');
  console.log('========================================================');
  ...
  for (const fid of priorityFondos) {
    ...
    for (const c of certsFondo) {
      ...
      // Calcular interés bruto en 59 días
    ...
    const txtFilename = path.join(reportsDir, `BCP_TELECREDITO_${fid}_2026-02-28.txt`);
    fs.writeFileSync(txtFilename, fullContent, 'utf-8');
    ...
    generatedSummary.push({
      ...
      detailSampleLength: detailLines[0].length
    });
  }

  console.log('\n--- RESUMEN DE ARCHIVOS TXT BCP GENERADOS ---');
  console.table(generatedSummary);
}

testGenerateBcpTxt().catch(err => {
