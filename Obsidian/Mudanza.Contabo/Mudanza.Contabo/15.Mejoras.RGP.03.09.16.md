...
| **8** | **Telecrédito BCP — Detección Inteligente de Renovaciones (Rollover)** | Prevención de fuga de capital en transferencias bancarias: discrimina entre contratos extinguidos (capital + interés) y renovados con correlativo `#0XX` (solo interés neto). | Frontend React + Motor Telecrédito BCP + Supabase |
...
### 📌 PUNTO 8: Discriminación Inteligente de Renovaciones (#0XX) vs. Devolución de Capital en Telecrédito BCP

#### A. Diagnóstico y Principio Financiero (BEN / LEG)
1. **La Paradoja del Vencimiento Tradicional:**
   - Cuando un contrato llega a su `fecha_fin` (vencimiento), el motor de liquidaciones estándar asume que el contrato se extingue y programa la **devolución del 100% del Capital Principal** al inversionista vía transferencia bancaria (Telecrédito BCP), sumado a sus intereses netos del período.
2. **La Realidad Operativa del Negocio (Rollover de Capital):**
   - En InAndes, la gran mayoría de partícipes que vencen **deciden renovar inmediatamente su inversión** para continuar en el fondo, creándose un nuevo contrato que reutiliza su **mismo número correlativo histórico (`#0XX`)**.
3. **Riesgo Operativo Crítico (Fuga Involuntaria de Liquidez):**
   - Si el sistema genera el archivo de Telecrédito BCP transfiriendo el capital de un contrato vencido que en realidad fue **renovado**, el fondo transferirá indebidamente el dinero al banco del cliente a pesar de que el cliente ya tiene un nuevo contrato activo por ese mismo capital.
...
flowchart TD
    ...
    CheckVen --> CheckRenov{"¿Existe Contrato Sucesor para este Inversionista en este Fondo?<br/>(Mismo Correlativo #0XX o fecha_inicio >= fecha_fin_anterior y estado ACTIVO)"}
    
    CheckRenov -- "SÍ (RENOVACIÓN CONFIRMADA)" --> Rollover["🔄 ROLLOVER DE CAPITAL (Permanece en el Fondo)<br/>• Monto Transferencia = ÚNICAMENTE Intereses Netos<br/>• Capital Principal = Retenido / Reinvertido en nuevo contrato"]
    Rollover --> BadgeRollover["🏷️ Badge UI: '🔄 Renovación #0XX (Solo Rendimiento)'"]
    
    CheckRenov -- "NO (EXTINCIÓN / SALIDA REAL)" --> Devolucion["🚪 DEVOLUCIÓN DE CAPITAL POR RETIRO<br/>• Monto Transferencia = Capital Principal Vivo + Intereses Netos<br/>• Certificado = Liquidado a USD/PEN 0.00"]
    Devolucion --> BadgeExtincion["🏷️ Badge UI: '🚪 Liquidación Total (Capital + Rendimiento)'"]
    
    BadgeRollover --> Telecredito[📄 Generación de Archivo TXT Telecrédito BCP / Planilla]
    BadgeExtincion --> Telecredito
```

#### C. Reglas de Validación para la Decisión de Transferencia:
1. **Detección por Correlativo Histórico (`#0XX`):**
   ...
   - Se consulta si existe en `crm_contratos` otro registro para el mismo `id_inversionista_1` y `id_fondo` con correlativo `17` y `estado IN ('emitido', 'activo', 'vigente')`.
2. **Cálculo del Importe Líquido para Telecrédito BCP:**
   $$\text{Monto Telecrédito} = \begin{cases} \text{Interés Neto Disponible} & \text{si existe Renovación Sucesora Activa} \\ \text{Capital Vivo} + \text{Interés Neto Disponible} & \text{si el Contrato se Extingue Definitivamente} \end{cases}$$
3. **Visibilidad en el Centro de Transferencias y Bot WhatsApp:**
   - En la tabla de transferencias BCP y en los mensajes enviados al partícipe, se detalla explícitamente el concepto liquidado (evitando que el cliente asuma que su capital no fue renovado o que el fondo transfiera fondos por error).
