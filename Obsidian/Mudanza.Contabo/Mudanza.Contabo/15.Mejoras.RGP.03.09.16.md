...
| **8** | **Telecrédito BCP — Renovaciones Inteligentes y Diferencial de Capital** | Cálculo automático del diferencial $(\text{Cap}_{\text{anterior}} - \text{Cap}_{\text{nuevo}})$ en renovaciones `#0XX`: transfiere solo intereses si rollover total, o diferencial + intereses si rollover parcial. | Frontend React + Motor Telecrédito BCP + Supabase |
...
### 📌 PUNTO 8: Discriminación Inteligente de Renovaciones (#0XX) y Diferencial de Capital en Telecrédito BCP

#### A. Diagnóstico y Principio Financiero (BEN / LEG)
1. **La Paradoja del Vencimiento Tradicional:**
   - Cuando un contrato llega a su `fecha_fin` (vencimiento), el motor de liquidaciones estándar asume que el contrato se extingue y programaría la **devolución del 100% del Capital Principal** al inversionista vía transferencia bancaria (Telecrédito BCP), sumado a sus intereses netos del período.
2. **La Realidad Operativa del Negocio (Rollover Total o Parcial de Capital):**
   - En InAndes, la gran mayoría de partícipes que vencen **deciden renovar su inversión** para continuar en el fondo, creándose un nuevo contrato que reutiliza su **mismo número correlativo histórico (`#0XX`)**.
   - Sin embargo, **el monto del nuevo contrato puede variar**:
     - **Caso A (Rollover Total o con Aumento):** El cliente renueva por el mismo monto o más ($\text{Capital Nuevo} \ge \text{Capital Anterior}$).
     - **Caso B (Rollover Parcial / Reducción de Capital):** El cliente renueva por un monto menor (ej. de $100,000 pasa a $80,000). En este caso, el fondo debe devolverle el **diferencial de capital** ($20,000).
     - **Caso C (Extinción Total / Retiro Definitivo):** El cliente no renueva. El fondo devuelve el 100% de su capital.
3. **Riesgo Operativo Crítico (Fuga o Bloqueo de Liquidez):**
   - Si el sistema no lee el capital del nuevo contrato sucesor, podría transferir el capital completo por error (fuga de liquidez) o no transferir la porción de capital liberada que el cliente solicitó retirar.

#### B. Arquitectura Técnica y Solución en Código (DIFF)

```mermaid
flowchart TD
    Init[📅 Generación de Transferencias Telecrédito BCP al Cierre] --> CheckVen[🔍 Detectar Contratos con fecha_fin en el período de corte]
    CheckVen --> CheckRenov{"¿Existe Contrato Sucesor para este Inversionista en este Fondo?<br/>(Mismo Correlativo #0XX con fecha_inicio >= fecha_fin_anterior y estado ACTIVO)"}
    
    CheckRenov -- "NO (EXTINCIÓN TOTAL)" --> DevolucionTotal["🚪 EXTINCIÓN / SALIDA DEFINITIVA<br/>• Capital a Transferir = 100% Capital Anterior<br/>• Monto Transferencia = Capital + Intereses Netos"]
    DevolucionTotal --> BadgeExtincion["🏷️ Badge UI: '🚪 Liquidación Total (Capital + Rendimiento)'"]

    CheckRenov -- "SÍ (RENOVACIÓN DETECTADA)" --> CompareCap{"Comparar Capitales:<br/>Capital Anterior vs. Capital Nuevo Contrato"}
    
    CompareCap -- "Cap. Nuevo >= Cap. Anterior" --> RolloverTotal["🔄 ROLLOVER TOTAL / INCREMENTO<br/>• Capital a Transferir = $0.00<br/>• Monto Transferencia = ÚNICAMENTE Intereses Netos"]
    RolloverTotal --> BadgeRollover["🏷️ Badge UI: '🔄 Renovación #0XX (Solo Rendimiento)'"]
    
    CompareCap -- "Cap. Nuevo < Cap. Anterior" --> RolloverParcial["✂️ ROLLOVER PARCIAL CON DIFERENCIAL<br/>• Capital a Transferir = Cap. Anterior - Cap. Nuevo<br/>• Monto Transferencia = Diferencial de Capital + Intereses Netos"]
    RolloverParcial --> BadgeParcial["🏷️ Badge UI: '✂️ Renovación #0XX (Diferencial Cap. + Rendimiento)'"]
    
    BadgeExtincion --> Telecredito[📄 Generación de Archivo TXT Telecrédito BCP / Planilla]
    BadgeRollover --> Telecredito
    BadgeParcial --> Telecredito
```

#### C. Reglas de Validación y Fórmulas Matemáticas de Negocio:
1. **Detección por Correlativo Histórico (`#0XX`):**
   - Se extrae el correlativo del contrato vencido (ej. de `NSGPEN01-017.20160101` $\rightarrow$ correlativo `17`).
   - Se consulta en `crm_contratos` si existe un contrato sucesor activo para el mismo `id_inversionista_1`, `id_fondo` y correlativo `17` con `fecha_inicio >= fecha_fin_anterior`.
2. **Fórmula Matemática del Capital a Transferir:**
   $$\text{Capital a Devolver} = \begin{cases} 
   \max\Big(0,\ \text{Capital Anterior} - \text{Capital Nuevo}\Big) & \text{si existe Contrato Renovado Sucesivo (\#0XX)} \\ 
   \text{Capital Anterior Vivo} & \text{si el Contrato se Extingue sin Renovación} 
   \end{cases}$$
3. **Monto Total a Transferir vía Telecrédito BCP:**
   $$\text{Monto Telecrédito Total} = \text{Interés Neto Devengado} + \text{Capital a Devolver}$$
4. **Visibilidad en el Centro de Transferencias y Bot WhatsApp:**
   - En la tabla de transferencias BCP y en los mensajes enviados al partícipe, se desglosa claramente:
     - `Intereses Netos del Período`
     - `Diferencial de Capital / Capital Devuelto` (si aplica)
     - `Monto Total a Transferir`
     - `Capital Reinvertido en Nuevo Contrato` (para transparencia total con el inversionista).
