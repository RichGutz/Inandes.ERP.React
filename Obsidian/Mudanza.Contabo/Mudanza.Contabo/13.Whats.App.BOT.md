...
stateDiagram-v2
    ...
    MenuInicial --> Opcion1_EECC: Opción 1 (EECC)
    MenuInicial --> Opcion2_Retencion: Opción 2 (Retención)
    MenuInicial --> Opcion3_Abono: Opción 3
    MenuInicial --> Opcion4_Asesor: Opción 4
    MenuInicial --> Opcion5_Salir: Opción 5
    
    Opcion1_EECC --> SubmenuPeriodosEECC: Múltiples periodos cerrados
    SubmenuPeriodosEECC --> DespachoPDF_EECC: Selecciona Periodo (1-N o Todos)
    SubmenuPeriodosEECC --> MenuInicial: Volver al menú
    DespachoPDF_EECC --> MenuSeguimiento: Envía Resumen + PDF Periodo

    Opcion2_Retencion --> SubmenuPeriodosRet: Múltiples periodos fiscales
    SubmenuPeriodosRet --> DespachoPDF_Ret: Selecciona Periodo (1-N o Todos)
    SubmenuPeriodosRet --> MenuInicial: Volver al menú
    DespachoPDF_Ret --> MenuSeguimiento: Envía Resumen + PDF Retención Firmado
...
## 9. 📊 Submenús Dinámicos de Períodos, Trazabilidad Contable y Co-Titularidad (Actualización AS BUILT)

### 9.1 Flujo de Navegación por Submenús Interactivos (EECC y Retenciones)
Para evitar envíos ciegos o predeterminados a un solo período, el bot implementa un **árbol de navegación determinista con submenús dinámicos**:

1. **Selección en Menú Principal:**
   - **Opción 1:** Estado de Cuenta Consolidado (EECC).
   - **Opción 2:** Certificado de Retención de Renta (2da Categoría).
2. **Construcción Dinámica del Submenú:**
   - El bot consulta en `crm_certificados_eventos` los eventos de cierre registrados para el contrato del partícipe (`tipo_evento in ['cierre_fin_ciclo', 'cierre_fin_contrato']`) ordenados descendentemente.
   - Presenta las opciones cronológicas con rango de fechas:
     ```text
     📊 SELECCIONA EL PERÍODO DEL ESTADO DE CUENTA (EECC):

     1. 📅 Cierre Agosto 2026 (01-07-2026 al 31-08-2026)
     2. 📅 Cierre Junio 2026 (01-05-2026 al 30-06-2026)
     3. 📅 Cierre Abril 2026 (01-03-2026 al 30-04-2026)
     4. 📅 Cierre Febrero 2026 (01-01-2026 al 28-02-2026)
     5. ↩️ Volver al Menú Principal

     👉 Responde con el número (1 - 5):
     ```
3. **Control de Flujo:**
   - La opción de retorno permite volver al menú principal en cualquier momento sin perder la sesión autenticada.

---

### 9.2 Origen Estricto de Datos: Ledger Contable del ERP (`crm_certificados_eventos`)
Todo valor numérico, cálculo financiero y retención tributaria proviene **exclusivamente de los asientos generados por el ERP al cierre de cada período**, garantizando 100% de coherencia contable:

| Campo en `crm_certificados_eventos` | Rol en el Documento Oficial |
| :--- | :--- |
| `capital_base` | Monto inicial invertido del período / Capital base de cálculo |
| `interes_generado_bruto` | Ganancia bruta devengada (Base de retención SUNAT) |
| `impuestos_renta` | Monto retenido por Impuesto a la Renta de 2da Categoría (5%) |
| `monto_reparto` / `interes_neto_disponible` | Ganancia disponible neta transferida a cuenta bancaria |
| `monto_deduccion` | Deducciones o comisiones aplicadas |
| `capital_final_saldo` | Saldo de capital acumulado al cierre |
| `fecha_periodo_origen` / `fecha_periodo_fin` | Rango de fechas oficial del ciclo contable |
| `id_certificado` | Numeración oficial del certificado |

---

### 9.3 Motor de Co-Titularidad (`resolve_contract_holders`)
Para contratos con múltiples titulares (ej. Titular Principal + Copartícipe), el bot no asume que quien chatea es el único titular. La función `resolve_contract_holders` resuelve la relación formal completa del contrato:

```python
def resolve_contract_holders(c: dict) -> dict:
    sb = get_supabase()
    holder_keys = ['id_inversionista_1', 'id_inversionista_2', 'id_inversionista_3']
    holders = []
    for k in holder_keys:
        inv_id = c.get(k)
        if inv_id:
            clean_id = str(inv_id).replace("DNI", "").strip()
            res = sb.from_('crm_inversionistas').select('*').or_(
                f"codigo_inversionista.eq.{inv_id},documento_identidad.eq.{clean_id},codigo_inversionista.eq.DNI{clean_id}"
            ).execute()
            if res.data:
                holders.append(res.data[0])
    
    nombres = [h.get('nombre_completo', '') for h in holders if h.get('nombre_completo')]
    dnis = [f"{h.get('tipo_doc', 'DNI')} {h.get('documento_identidad', '')}" for h in holders if h.get('documento_identidad')]
    dir_fiscal = holders[0].get('direccion_fiscal') if (holders and holders[0].get('direccion_fiscal')) else "Calle Lorca 170 Dpto 401 La Castellana, Santiago de Surco, Lima"
    
    return {
        'titulares_str': " / ".join(nombres) if nombres else "Partícipe",
        'dnis_str': " / ".join(dnis) if dnis else "",
        'direccion_fiscal': dir_fiscal,
        'titular_principal': holders[0] if holders else None,
        'all_holders': holders
    }
```

#### Formato en Documentos:
- **Encabezado EECC:** `Sr(a)(s): León Vigo Nancy Edelmira / Gutiérrez León Richard Eduardo`
- **Certificado de Retención:**
  > *"De acuerdo con la LEY DEL IMPUESTO A LA RENTA, se le(s) ha(n) retenido al(a) Sr(a). **León Vigo Nancy Edelmira / Gutiérrez León Richard Eduardo**, identificado(a) con Documento **DNI 08810864 / DNI 09870156** y con domicilio fiscal en **Calle Lorca 170 Dpto 401 La Castellana, Santiago de Surco, Lima**..."*

---

### 9.4 Sandbox de Ensayo (Whitelist de 3 DNIs) y Modo Demostración
Para garantizar seguridad operacional durante la fase de validación de entrega de documentos oficiales:

1. **Lista Blanca Autorizada (Documentos Reales con Firma de Gerencia):**
   - `09870156`: Richard Eduardo Gutiérrez León
   - `02816271`: Juan Ricardo Gallo Pizarro
   - `000949712`: Gladys Yanneth Parra Forero
2. **Modo Demostración / Fallback para Terceros:**
   - Cualquier otro DNI o número recibe un PDF seguro de demostración con Logo institucional y aviso formal de conciliación contable/auditoría.
