...
## 🔎 Caso Pericial N° 11: Calibración Definitiva del Motor NAV V30 (Aumentos de Capital 1:1 con Retornos V40 y Ganancia Operativa = 0.00)

**Fecha**: 29 de Agosto de 2026  
**Investigador**: Detective Benoit Blanc  
**Módulos**:
1. Motor de Retornos y Rendimientos ([`financialCalculator.ts`](file:///c:/Users/rguti/Inandes.ERP.React/src/utils/financialCalculator.ts))
2. Servicio de Cálculo de Fondos ([`fondosService.ts`](file:///c:/Users/rguti/Inandes.ERP.React/src/services/fondosService.ts) $\rightarrow$ `calculateValorCuotaV30`)
3. Generador de Excel Maestro V30 ([`excelGeneratorValorCuotaV30.ts`](file:///c:/Users/rguti/Inandes.ERP.React/src/utils/excelGeneratorValorCuotaV30.ts))
4. Generador de PDF Oficial V30 ([`pdfGeneratorValorCuotaV30.ts`](file:///c:/Users/rguti/Inandes.ERP.React/src/utils/pdfGeneratorValorCuotaV30.ts))
5. Vista de Fondos y Tasas ([`FondosPage.tsx`](file:///c:/Users/rguti/Inandes.ERP.React/src/features/fondos/FondosPage.tsx))
6. Script de QC Oficial ([`scripts/qc_motor_nav_v30_pl_zero.py`](file:///c:/Users/rguti/Inandes.ERP.React/scripts/qc_motor_nav_v30_pl_zero.py))

---

### 🩸 11.1. La Escena del Crimen (`LEG` - Legacy)

* **Disparidad de Serialización y Salto de Aumentos**:
  * En `financialCalculator.ts`, `fecha_inicio` se emitía en formato textual corto `"02/01"`.
  * En `fondosService.ts`, `new Date("02/01")` arrojaba `Invalid Date` (`NaN`), provocando que la condición de coincidencia temporal diaria (`h.fecha_ingreso.getTime() === d.getTime()`) fuera permanentemente falsa.
  * Los aumentos de capital de enero (ej. `NSGPEN01-090`: S/ 60k al 02/01, S/ 9k al 03/01, S/ 100k al 12/01) no se incorporaban en `(+) CAPITAL ADICIONAL (Hoy)` ni emitían nuevas cuotas en el Motor NAV.

---

### ⚖️ 11.2. Autopsia de Diferencias (`DIFF`)

```diff
- [LEGACY V28/V29: Fechas Rotas y Aportes Omitidos]
- Retornos: fecha_inicio: "02/01" (Sin año, parseaba NaN)
- NAV: apD = 0.00 en todos los días con aumentos
- Cuotas Adicionales: 0 emitidas
- Ganancia Operativa: Disparidades residuales

+ [MOTOR NAV V30: Consumo Directo 1:1 y Ganancia Operativa = 0.00 Estricto]
+ 1. src/utils/financialCalculator.ts:
+    • Emisión de ISO Strings universales (fecha: h.fecha.toISOString(), fecha_exacta).
+
+ 2. src/services/fondosService.ts (calculateValorCuotaV30):
+    • Parser universal parseFechaIngreso(r, startDate).
+    • Comparador de calendario isSameDay(h.fecha_ingreso, d).
+    • Captura de Aportes Diarios: 02/01 (60,000), 03/01 (9,000), 12/01 (100,000).
+    • Emisión exacta de Cuotas: nuevasCuotas = INT(Aporte / VC_Ayer).
+    • Identidad P&L Cero: Ingreso Bruto = Egresos (Ganancia Operativa = $ 0.00 cada día).
+
+ 3. Reportes Oficiales V30:
+    • excelGeneratorValorCuotaV30.ts: Pestañas por fondo con banner P&L = 0.00.
+    • pdfGeneratorValorCuotaV30.ts: Quincenal A4 Landscape en 2 páginas con badge V30.
```

---

### 🧪 11.3. Certificación de Convergencia (`NOTA`)

1. **Loop QC Pericial Terminal**: `python scripts/qc_motor_nav_v30_pl_zero.py`
   - `NSGPEN01`: Suma Ganancia Operativa = **`$ 0.00 (CERO ESTRICTO)`** | $\text{VC al 28/02} = \mathbf{1.02187407}$ ✅
   - `NSGPEN02`: Suma Ganancia Operativa = **`$ 0.00 (CERO ESTRICTO)`** | $\text{VC al 28/02} = \mathbf{1.02059079}$ ✅
   - `NSGPEN03`: Suma Ganancia Operativa = **`$ 0.00 (CERO ESTRICTO)`** | $\text{VC al 28/02} = \mathbf{1.02120841}$ ✅
   - `NSGUSD01`: Suma Ganancia Operativa = **`$ 0.00 (CERO ESTRICTO)`** | $\text{VC al 28/02} = \mathbf{1.01863841}$ ✅
   - `NSGUSD02`: Suma Ganancia Operativa = **`$ 0.00 (CERO ESTRICTO)`** | $\text{VC al 28/02} = \mathbf{1.01800418}$ ✅
   - **Tasa de Aprobación**: **`295 / 295 (100.00%)`**.
2. **Compilación y Despliegue**:
   - `npm run build`: `✓ built in 3.10s (0 errores)`.
   - Commit: `6b3c9b3` empujado a `origin/main` para auto-despliegue en VPS Contabo Coolify.
   - Sonda Guardian Systemd: **`HTTP 200 OK`**.

---

*Caso N° 11 cerrado y sellado por Detective Benoit Blanc - 29.08.2026.*
