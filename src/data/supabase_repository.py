...
def create_crm_deduccion(data: Dict) -> tuple[bool, str]:
    try:
        client = get_supabase_client()
        response = client.table('crm_deducciones').insert(data).execute()
        if response.data:
            return True, "Deducción creada exitosamente"
        return False, "No se pudo crear la deducción"
    except Exception as e:
        return False, str(e)

def get_crm_deducciones_by_contrato(id_contrato: str) -> List[Dict]:
    try:
        client = get_supabase_client()
        response = client.table('crm_deducciones').select('*').eq('id_contrato', id_contrato).order('prioridad_orden').execute()
        return response.data if response.data else []
    except Exception as e:
        print(f"Error obteniendo deducciones para contrato {id_contrato}: {e}")
        return []

def add_crm_deduccion_evento(data: Dict) -> tuple[bool, str]:
    try:
        client = get_supabase_client()
        response = client.table('crm_deducciones_eventos').insert(data).execute()
        if response.data:
            return True, "Evento insertado exitosamente"
        return False, "No se pudo insertar evento"
    except Exception as e:
        return False, str(e)

# --- CRONOGRAMA UNIFICADO DE DEDUCCIONES Y RESCATES ---

def insert_cronograma_deducciones(data_list: List[Dict]) -> tuple[bool, str]:
    """Inserta una lista de cuotas/armadas en la tabla crm_cronograma_deducciones_rescates."""
    if not data_list:
        return True, "Ningún dato para insertar."
    try:
        client = get_supabase_client()
        response = client.table('crm_cronograma_deducciones_rescates').insert(data_list).execute()
        if hasattr(response, 'data') and response.data:
            return True, f"Se insertaron {len(response.data)} filas exitosamente."
        return False, f"No se pudo insertar el cronograma: {getattr(response, 'error', 'Error desconocido')}"
    except Exception as e:
        return False, str(e)

def get_cronograma_deducciones_by_contrato_or_cert(id_texto: str) -> List[Dict]:
    """Busca los cargos pendientes o activos para un contrato o certificado en la nueva tabla."""
    try:
        client = get_supabase_client()
        response = client.table('crm_cronograma_deducciones_rescates').select('*').or_(f"id_contrato.eq.{id_texto},id_certificado.eq.{id_texto}").order('fecha_proyectada_cobro').execute()
        return response.data if response.data else []
    except Exception as e:
        print(f"Error obteniendo cronograma para {id_texto}: {e}")
        return []

def get_pending_cronograma_by_certificates(cert_ids: List[str]) -> List[Dict]:
    """Busca todos los cargos PENDIENTES para una lista de certificados."""
    if not cert_ids:
        return []
    try:
        client = get_supabase_client()
        # Dividir en chunks si la lista es muy larga (Supabase tiene limites en IN)
        all_data = []
        for i in range(0, len(cert_ids), 100):
            chunk = cert_ids[i:i+100]
            response = client.table('crm_cronograma_deducciones_rescates').select('*').in_('id_certificado', chunk).eq('estado', 'PENDIENTE').execute()
            if response.data:
                all_data.extend(response.data)
        return all_data
    except Exception as e:
        print(f"Error masivo en cronograma: {e}")
        return []

def update_cronograma_ejecutado(id_cuota: str, id_evento_ledger: str) -> bool:
    """Marca una cuota como EJECUTADA y la vincula al evento del Ledger."""
    try:
        client = get_supabase_client()
        response = client.table('crm_cronograma_deducciones_rescates').update({
            'estado': 'EJECUTADA',
            'id_evento_ledger': id_evento_ledger
        }).eq('id_cuota', id_cuota).execute()
        return bool(response.data)
    except Exception as e:
        print(f"Error actualizando cuota {id_cuota}: {e}")
        return False
