...
/**
 * Generador Oficial de Excel Maestro para Valor Cuota NAV V31
 * Incorpora la secuencia contable canónica V26 (Pre-Aportes, Cierre y Goal Seek P&L = 0.00)
 */
export async function generateValorCuotaExcelV31(options: VcExcelOptions): Promise<void> {
  const { reports, selYear, fStart, fEnd } = options;

  if (!reports || reports.length === 0) {
    throw new Error("No hay reportes de Valor Cuota V31 disponibles para exportar.");
  }

  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'INANDES GRUPO FINANCIERO';
  workbook.lastModifiedBy = 'InAndes React CRM (Motor NAV V31)';
  ...
  for (const report of reports) {
    ...
    const titleRow = ws.addRow([
      `FONDO ${report.fondo.nombre_fondo || report.fondo.id_fondo} (${report.fondo.id_fondo}) — MOTOR NAV V31 (P&L = 0.00 IDENTIDAD CERO)`
    ]);
    titleRow.height = 24;
    ...
    for (let rIdx = 0; rIdx < primaryBlock.rows.length; rIdx++) {
      ...
      row.eachCell((cell, colNumber) => {
        ...
        if (colNumber === 1) {
          ...
          if (isTotal) {
            if (sampleRow.id === 'TOTAL CAPITAL' || sampleRow.id === 'PATRIMONIO TOTAL CIERRE') {
              cell.fill = {
                type: 'pattern',
                pattern: 'solid',
  ...
  a.download = `EXCEL_MAESTRO_VALOR_CUOTA_NAV_V31_${selYear}_${timeStr}.xlsx`;
