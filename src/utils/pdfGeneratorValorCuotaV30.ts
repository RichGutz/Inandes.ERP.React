...
/**
 * Generador Oficial de Reporte PDF para Valor Cuota NAV V30
 ...
 */
export function generatePdfValorCuotaV30(options: VcPdfOptions): string {
    ...
    for (const block of report.blocks) {
      ...
      const renderPagePart = (
        ...
        for (const row of block.rows) {
          ...
          const isAumento = row.id && row.id.includes('Aumento');
          const isTotal = row.tipo === 'TOTAL';
          const isVc = row.is_vc || row.id.includes('VAL CUOTA');
          const isGananciaOp = row.id === 'GANANCIA OPERATIVA (Neta)';
          const isTotalApertura = row.id === 'TOTAL CAPITAL (Apertura)' || row.id === 'CUOTAS APERTURA' || row.id === 'PATRIMONIO TOTAL (Pre-Aportes)';
          ...
          <div class="sheet-page">
            <div class="header-container">
              ...
              <div class="header-right">
                <span class="badge-v30">NAV V30 (P&L = 0.00)</span>
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Reporte Oficial Valor Cuota NAV V30</title>
      <style>
        @page {
          size: A4 landscape;
          margin: 4mm 5mm;
